...
const artifactPath = path.join(browserDir, 'artifacts', 'phase-2d3c-targeted-results.json');

const files = new Map([
  ['/', [path.join(browserDir, 'phase-2d3c-targeted.html'), 'text/html; charset=utf-8']],
  ['/phase-2d2-runtime.js', [path.join(browserDir, 'phase-2d2-runtime.js'), 'text/javascript; charset=utf-8']],
  ['/phase-2d3c-targeted.js', [path.join(browserDir, 'phase-2d3c-targeted.js'), 'text/javascript; charset=utf-8']],
...
server.listen(8096, '127.0.0.1', () => {
  process.stdout.write('Phase 2D3C targeted probe: http://127.0.0.1:8096/\n');
});
